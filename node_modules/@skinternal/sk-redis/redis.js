...
const RedisConnector = function (opts) {
  ...
  if(opts.queuePacket && opts.queueConcurrency >= 1){
    this.concurrency = opts.queueConcurrency
  }

  if(opts.batchProcess){
  ...
  const subClientInstanceId = generateHash()
  if(global.secrets.uniqueIdentityMode){
    subClient.subscribe(opts.identity + '_' + subClientInstanceId);
  }else {
          ...
          if (now - packet.timestamp > timeout) {
            ...
            if (packet.respondTo) {
              if (packet.respondTo.publish) {
                self.client.publish(global.secrets.uniqueIdentityMode?packet.respondTo.to + '_' + packet.respondTo.fromInstance:packet.respondTo.to, JSON.stringify({
                  ...
                  fromInstance: packet.respondTo.fromInstance
                }));
              } else {
                let newPacket
                if (err) {
              ...
              self.client.publish(global.secrets.uniqueIdentityMode?packet.from + '_' + packet.fromInstance:packet.from, JSON.stringify({
                to: packet.from,
                from: packet.to,
                timestamp: new Date().getTime(),
                originalTimestamp: packet.timestamp,
                ...
                tid: packet.tid,
                fromInstance: packet.fromInstance
              }));
            }
          }
        }
  ...
  const processPacketsBatch = (packets) =>{
    return new Promise((resolve,reject) => {
      if (!packets) {
        return resolve()
      }
      self.emit('messages', packets, (err, res) => {
        resolve()
      });
    });
  }
          ...
          .exec(async (err, data) => {
            ...
            try {
              ...
              if(!opts.batchProcess){
                ...
                await processPacketsBatch(packets)
              }
  ...
  this.sendSync = function (to, protocol, payload, customTimeout,callback) {
    var packet = {
      ...
      payload,
      tid: generateHash(),
      fromInstance:subClientInstanceId
    };
