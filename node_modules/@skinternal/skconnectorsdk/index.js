...
//**** Web Server Related End ****//
    ...
    upload = null,
    proxyMode = false,
    initializeListName = null,
    initializeClient = null,
    initializeOpts = null,
    initializeRegisteredMethods = null,
    {proxyMessage} = require('./modules/httpClient')
methods = () =>{};
global.Redis = null;
global.proxyInitialized = false

const proxyBlacklistMethods =     [
  'respondTouser',
  'sendActionUserView',
  'sendAction',
  'sendActionNoResponse',
  'redisSet',
  'redisGet',
  'redisRemove',
    'connectorProcessCommand'
]
...
const startWebServer = async () =>{
    ...
    const listenResponse = await httpServerListen(identity,'localhost',process.env.SK_PROXY_SERVER_PORT?process.env.SK_PROXY_SERVER_PORT:9292)
}
            ...
            if (packet.protocol === 'action') {
                if (!methods[funcName] && connectorManifest && connectorManifest.capabilities && connectorManifest.capabilities[packet.payload.capabilityName] && connectorManifest.capabilities[packet.payload.capabilityName].respondToUser) {
                    ...
                    if (methods[`handle_capability_${packet.payload.capabilityName}`]) {
                        if(proxyMode) {
                            //Send message to connector microservice
                            response = await proxyMessage({funcName:`handle_capability_${packet.payload.capabilityName}`,funcArgs:[packet.payload]});
                        }else {
                        ...
                        if(proxyMode){
                            //Send message to connector microservice
                            response = await proxyMessage({funcName:`processEvent`,funcArgs:[packet]});
                        }else {
                ...
                if (methods[funcName]) {
                    if(proxyMode) {
                        //Send message to connector microservice
                        response = await proxyMessage({funcName, funcArgs: [packet.payload]});
                    }else {
...
const queueProcessBatch = async (packets,callback, httpCallback) =>{
    return processPacketsBatch(packets, callback, httpCallback);
}
...
const processPacketsBatch = async (packets, callback,httpCallback) => {
    try {
        const response = await methods.processEventsBatch(packets)
...
const processCommand = async (commandObj) =>{
    ...
    try {
        ...
        if(parsedMessage.command === 'invalidateConnectionFromCache') {
        ...
        } else if(parsedMessage.command === 'invalidateCompanyFromCache') {
            companyDAO.invalidateCache(parsedMessage.companyId)
        }else if(parsedMessage.command === 'refreshSecrets') {
            ...
            methods.connectorProcessCommand(parsedMessage,commandObj.channel,commandObj.channelPattern)
        }else if(parsedMessage.command === 'invalidateConnectorFromCache') {
            const { connectorId } = parsedMessage
            console.log('invalidateConnectorFromCache')
            console.log(connectorId)
            connectorsDAO.invalidateConnectorFromCache(connectorId)
        } else if(parsedMessage.command === 'invalidateAllConnectorFromCache') {
            const { connectorId } = parsedMessage
            console.log('invalidateAllConnectorFromCache')
            console.log(connectorId)
            connectorsDAO.invalidateAllConnectorFromCache()
        ...
        }else if(parsedMessage.command === 'initializeService') {
            console.log('initializing service')
            //TODO: Ensure that redis, mongodb, and other connections are terminated before reinitializing
        //    await module.exports.initialize(initializeListName,initializeClient,initializeOpts,initializeRegisteredMethods)
...
methods.processEventsBatch = async (packets) => {
    throw {success: false, message: "Method processEventsBatch not implemented"}
}
...
methods.sendActionUserView = async (packet) => {
    ...
    Redis.sendToList(packet.to, packet.protocol, packet.payload, {to: 'api', tid: packet.tid, fromInstance: packet.fromInstance, publish: true})
    if (logUserEvents) {
...
const getHttpClient = function() {
  return httpClient
}

const initalize = async (listName,client,opts,registeredMethods) => {
    identity = listName
    globalListName = listName
    initializeListName = listName
    initializeClient = client
    initializeOpts = opts
    initializeRegisteredMethods = registeredMethods
    ...
    //When the client calls initialize method
    if(proxyMode && Array.isArray(registeredMethods)){
        registeredMethods.forEach(k=>{
            if(!proxyBlacklistMethods.includes(k)){
                methods[k] = 'proxy'
            }
        })
...
//    globalListName = `${listName}_${os.hostname()}_${process.env.NODE_APP_INSTANCE != undefined?process.env.NODE_APP_INSTANCE:0}`
        ...
        connectorsDAO = require('./data/connectorsDAO')
        ...
        const httpClientOptions = opts ? opts.httpClientOptions || {} : {};

        const httpClientModule = require('./modules/httpClient');
        httpClient = httpClientModule.initialize(httpClientOptions.config, {
          requestInterceptor: httpClientOptions.requestInterceptor,
          requestErrorInterceptor: httpClientOptions.requestErrorInterceptor,
          responseInterceptor: httpClientOptions.responseInterceptor,
          responseErrorInterceptor: httpClientOptions.responseErrorInterceptor,
        });
        ...
        if (currentConfig) {
            ...
            if(typeof rateLimit.batchProcess === 'boolean'){
                queueOpts.batchProcess = rateLimit.batchProcess
            }else if(typeof opts.batchProcess === 'boolean'){
                queueOpts.batchProcess = opts.batchProcess
            }
        ...
        Redis = new redisclient({ identity: globalListName ,...opts, receiveEventsFlag:typeof opts.receiveEventsFlag !== 'boolean'?false:opts.receiveEventsFlag,queuePacket: !!queueOpts.enabled,queueConcurrency:queueOpts.concurrency,batchProcess:queueOpts.batchProcess})
        global.Redis = Redis;
        Redis.on('message', queueProcessPacket);
        Redis.on('messages', queueProcessBatch);
...
const getUploadedFile = (fileName,companyId,connectionId,capabilityName) => {
    return upload.getUploadedFile(fileName,companyId,connectionId,capabilityName)
}
